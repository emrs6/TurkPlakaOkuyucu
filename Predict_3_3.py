...
picam2.set_controls({'Sharpness': 3, 'Contrast': 1.20, 'Saturation': 0.80, 'AeExposureMode': 1, 'ExposureValue': (-0.5) })
    ...
    for car_detection in car_detections_:
        ...
        if score > 0.4:
            ...
            if car is None or car.size == 0:
                continue
            car = resize_image_width(car, 600)
            # plakayı bul
            ...
            for result in results:
                if len(result.boxes.xywh) > 0:
                    ...
                    cropped_thresh = cv2.threshold(cropped_denoised, 127, 255, cv2.THRESH_BINARY_INV)[1]
                    ...
                    for i, box in enumerate(letterData):
                        ...
                        thresholed_letter = cv2.threshold(noise_reduce_letter, 127, 255, cv2.THRESH_BINARY_INV)[1]
                    ...
                    if letters and not all(image is None for image in letters):
                        ...
                        cv2.imwrite("rec.jpg", black_rectangle2)
