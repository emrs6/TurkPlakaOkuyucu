...
#picam2.set_controls({'Sharpness': 3, 'Contrast': 1.50, 'Saturation': 0.80, 'AeExposureMode': 1})
            ...
            for result in results:
                if len(result.boxes.xywh) > 0:
                    ...
                    if letters and not all(image is None for image in letters):
                        ...
                        black_rectangle2 = resize_image_width(black_rectangle2 , 750)
                        

                        reversed_rct = cv2.bitwise_not(black_rectangle2)

                        plt.imshow(reversed_rct)
                        plt.show()
                        ...
                        data = pytesseract.image_to_string(reversed_rct, lang="eng")
                        allowed_chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
                        sonuclar = "".join(c for c in data if c in allowed_chars)
                        if sonuclar is not None and len(str(sonuclar)) != 0:
                            plaka_verisi = str(sonuclar)
                        else:
                            plaka_verisi = ""
                        print(plaka_verisi)
                        if len(plaka_verisi) > 6:

                            # sonuç düzenleme

                            print("Çıkarılan plaka:")
                            plaka = str(plaka_verisi)
